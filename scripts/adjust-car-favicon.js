...
async function createCarFavicon() {
  try {
    ...
    // Valores para recorte (ajuste conforme necessário)
    // Valores em percentuais para serem mais consistentes
    const leftPct = 0.15;  // 15% da largura
    const topPct = 0.15;   // 15% da altura
    const widthPct = 0.25; // 25% da largura
    const heightPct = 0.4; // 40% da altura

    const left = Math.round(metadata.width * leftPct);
    const top = Math.round(metadata.height * topPct);
    const width = Math.round(metadata.width * widthPct);
    const height = Math.round(metadata.height * heightPct);
