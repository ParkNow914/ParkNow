...
class EstacionamentoPaymentProcessingService {
    ...
    async processarPagamentoPix(reserva, estacionamento, valorTotal, dadosPagamento, client) {
        ...
        // Gera o QR Code PIX (implementação simplificada)
        const qrCodeData = this.gerarQrCodePix({
            ...
            cidade: estacionamento.cidade || 'São Paulo',
    ...
    /**
     * Gera dados de QR Code PIX (implementação simplificada)
     * @private
     */
    gerarQrCodePix({ chavePix, nomeTitular, valor, descricao, cidade, txid }) {
        // Em produção, você usaria uma biblioteca ou API para gerar o QR Code PIX
        // Esta é uma implementação simplificada apenas para demonstração
        
        // Formata o valor para o padrão PIX (em centavos, sem separadores)
        const valorCentavos = Math.round(valor * 100);
        
        // Gera o payload do PIX (formato simplificado)
        const payload = [
            '000201', // Payload Format Indicator
            '2633', // Ponto de Iniciação Opcional
            '0014br.gov.bcb.pix', // GUI do Payload
            '01', // Chave PIX
            `${chavePix.length.toString().padStart(2, '0')}${chavePix}`, // Valor da Chave
            '52040000', // Código do País (Brasil)
            '5303986', // Moeda (Real)
            `54${valorCentavos.toString().length.toString().padStart(2, '0')}${valorCentavos}`, // Valor
            '5802BR', // Código do País da Entidade Comercial
            '59', // Nome do Beneficiário
            `${Math.min(nomeTitular.length, 25).toString().padStart(2, '0')}${nomeTitular.substring(0, 25)}`,
            '60', // Cidade do Beneficiário
            `${Math.min(cidade.length, 15).toString().padStart(2, '0')}${cidade.substring(0, 15)}`,
            '61', // CEP do Beneficiário (opcional)
            '00000000',
            '6207', // Adicionais
            '05', // Referência do Pagador
            '***',
            '6304', // CRC16
            'ABCD' // CRC16 (simulado)
        ].join('');
        
        // Gera uma URL para um serviço de QR Code (em produção, use uma biblioteca no frontend)
